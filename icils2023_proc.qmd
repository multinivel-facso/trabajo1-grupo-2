...
### Correlaciones con variables centradas

```{r}
datos_matriz_corr <- datos_analisis %>%  
  select(any_of(c("S_GENEFF", "S_SPECEFF","homelit_sch_gmc", "homelit_cmc","cil_sch_gmc", "cil_cmc", "desktops_cmc", "tablets_cmc", "idi_gmc"))) %>%
  mutate(across(everything(), as.numeric))
...
m1_gen <- lmer(S_GENEFF ~ S_SEX + S_HOMLIT + desktops_cmc + tablets_cmc + 
               (1 | CNTRY/id_school_unique), 
               data = datos_limpios)


# --- Modelo 2: Variables escolares ---
m2_gen <- lmer(S_GENEFF ~ S_SEX + S_HOMLIT + desktops_cmc + tablets_cmc +
               cil_sch_gmc +
               (1 | CNTRY/id_school_unique), 
               data = datos_limpios)
...
m3_gen <- lmer(S_GENEFF ~ S_SEX + S_HOMLIT + desktops_cmc + tablets_cmc +
              cil_sch_gmc + idi_gmc +
               (1 | CNTRY/id_school_unique), 
               data = datos_limpios)
...
El Homelit agrupado por escuela no es significativo. Centramos el IDI cuando nos dimos cuento que modificaba mucho el intercepto.
...
m4_gen <- lmer(S_GENEFF ~ cil_sch_gmc * S_SEX + S_HOMLIT + desktops_cmc + tablets_cmc +
               cil_sch_gmc + idi_gmc +
               (1 | CNTRY/id_school_unique), 
               data = datos_limpios)

sjPlot::tab_model(m1_gen, m2_gen, m3_gen, m4_gen)
...
El efecto de tener mayor alfabetización digital implica un
