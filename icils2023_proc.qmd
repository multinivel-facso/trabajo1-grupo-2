...
```{r}
# Grupos de variables
vars_categoricas <- c("S_HOMLIT", "S_SEX", "IS3G16BA", "IS3G16BB", 
                      "IS3G16BC", "IS3G16AA", "IS3G16AB", "S_INTNET")
vars_escala <- c("S_GENEFF", "S_SPECEFF", "C_RATDEV", "C_RATSTD", "C_ICTRES")
...
table1(~ S_GENEFF + S_SPECEFF + S_SEX + S_INTNET + S_HOMLIT, 
       data = datos_limpios,
...
label(datos_schl$C_ICTRES)  <- "coordinadores TIC informan disponibilidad recursos TIC en la escuela"

table1(~ C_RATDEV + C_RATSTD + C_ICTRES, 
       data = datos_schl,
...
label(datos_limpios$S_GENEFF)  <- "Autoeficacia General"
label(datos_limpios$S_SPECEFF) <- "Autoeficacia Específica"
label(datos_limpios$C_RATDEV)  <- "Ratio de Dispositivos por Escuela"
label(datos_limpios$C_RATSTD)  <- "Ratio de Dispositivos por Estudiante"
label(datos_limpios$C_ICTRES)  <- "coordinadores TIC informan disponibilidad recursos TIC en la escuela"


tabla3_continuas <- table1(~ S_GENEFF + S_SPECEFF + C_RATDEV + C_RATSTD + C_ICTRES, 
                           data = datos_limpios,
                           caption = "<h3>Tabla 3: Estadísticos Descriptivos de Variables Continuas</h3>",
...
ggplot(datos_limpios, aes(x = S_GENEFF , y = C_RATDEV)) +
...
ggplot(datos_limpios, aes(x = S_SPECEFF , y = C_RATDEV)) +
  geom_point(alpha = 0.1, color = "darkgray") +
  geom_smooth(method = "lm", color = "darkorange", se = FALSE) +
  labs(
...
ggplot(datos_limpios, aes(x = S_SPECEFF , y = C_ICTRES)) +
  geom_point(alpha = 0.1, color = "darkgray") +
  geom_smooth(method = "lm", color = "darkorange", se = FALSE) +
  labs(
    title = "Relación entre Autoeficacia Específica y Información Disponibilidad TIC Escuela",
    subtitle = "Cada punto representa a un estudiante",
    x = "Puntaje de Autoeficacia Específica del Estudiante",
    y = "Información Disponibilidad TIC Escuela"
  ) +
  theme_minimal()
```
...
datos_indice_acceso <- datos_limpios %>%  
  select(IS3G16BA, IS3G16BB, IS3G16BC, IS3G16AA, IS3G16AB) %>%
...
alfa_resultado <- alpha(datos_indice_acceso)

print(alfa_resultado)
```
...
results_0_gen_sch = lmer(S_GENEFF ~ 1 + (1 | id_school_unique), data = datos_limpios)
results_0_spec_sch = lmer(S_SPECEFF ~ 1 + (1 | id_school_unique), data = datos_limpios)
...
modelo_gen_intercepto <- lmer(S_GENEFF ~ IS3G16AA + IS3G16AB + S_HOMLIT + S_SEX + C_RATSTD + (1 | id_school_unique),
                              data = datos_limpios)


# --- Modelo 2: Autoeficacia ESPECÍFICA con Intercepto Aleatorio ---
modelo_spec_intercepto <- lmer(S_SPECEFF ~ IS3G16AA + IS3G16AB + S_HOMLIT + S_SEX + C_RATSTD + (1 | id_school_unique),
                               data = datos_limpios)


# --- Modelo 3: Autoeficacia GENERAL con Intercepto y Pendiente Aleatorios ---
modelo_gen_pendiente <- lmer(S_GENEFF ~ IS3G16AA + IS3G16AB + S_HOMLIT + S_SEX + C_RATSTD + (1 + IS3G16AA + IS3G16AB | id_school_unique),
                             data = datos_limpios)


# --- Modelo 4: Autoeficacia ESPECÍFICA con Intercepto y Pendiente Aleatorios ---
modelo_spec_pendiente <- lmer(S_SPECEFF ~ IS3G16AA + IS3G16AB + S_HOMLIT + S_SEX + C_RATSTD + (1 + IS3G16AA + IS3G16AB | id_school_unique),
                              data = datos_limpios)

# summary(modelo_gen_intercepto)
```
