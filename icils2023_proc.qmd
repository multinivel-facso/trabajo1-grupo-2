...
datos_schl <- datos_combinados_bcg %>%
  select(IDSCHOOL, C_RATDEV, C_RATSTD)
```

### Unimos dataframes

Hay escuelas que están en varios países, por lo que el ID es el mismo pero responden de forma separada en el cuestionario de escuela. Creo que podríamos promediar el ratio de dispositivos por estudiante por id escuela (group_by ID_SCHOOL) y mantener solo una fila. Por eso ICILS dice que se aplicó a 5200 escuelas y luego hay solo 608.

```{r}

# datos_analisis <- left_join(datos_stud, datos_schl, by="id_school")
```

-   Descriptivos univariados por nivel (recomiendo usar la función table1 del paquete table1)

-   Descriptivos bivariados por nivel (recomiendo usar la función table1 del paquete table1)

### Descriptivos univariados y bivariados

Agregar todas las variables y labels para las categorías de respuesta

```{r}
library(table1)

table1::table1(~IS3G16BA + IS3G16BB + IS3G16BC + IS3G16AA + IS3G16AB + S_SEX + S_GENEFF, data=datos_analisis)
```
...
# Dataframe dividido para facilitar alfa de cronbach y matriz de correlaciones

datos_analisis <- datos_analisis %>% 
  mutate(
    IS3G16BA = as.factor(IS3G16BA),
    IS3G16BB = as.factor(IS3G16BB),
    IS3G16BC = as.factor(IS3G16BC),
    IS3G16AA = as.factor(IS3G16AA),
    IS3G16AB = as.factor(IS3G16AB),
    S_SEX = as.factor(S_SEX)
  )
```
