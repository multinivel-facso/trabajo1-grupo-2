...
modelo_gen_pendiente <- lmer(S_GENEFF ~ desktops_cmc + tablets_cmc + C_RATSTD + IDI +       desktops_cmc*IDI + tablets_cmc*IDI + S_HOMLIT + S_SEX + (1 + desktops_cmc + tablets_cmc | CNTRY) + (1 | id_school_unique), data = datos_limpios, control = lmerControl(optimizer = "bobyqa"))

# --- Modelo 4: Autoeficacia ESPECÍFICA con Intercepto y Pendiente Aleatorios ---
modelo_spec_pendiente <- lmer(S_SPECEFF ~ desktops_cmc + tablets_cmc + C_RATSTD + IDI + desktops_cmc*IDI + tablets_cmc*IDI + S_HOMLIT + S_SEX + (1 + desktops_cmc + tablets_cmc | CNTRY) + (1 | id_school_unique),data = datos_limpios, control = lmerControl(optimizer = "bobyqa"))
...
estex_gen_cnt <- influence(modelo_gen_pendiente, "CNTRY") # ex estex_mod_est
